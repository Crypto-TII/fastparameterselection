...
for group_name, cols in groups.items():
    results[group_name] = {
        ...
        "overall_worst": (0, None, None, None, None),  # (diff, row, col, file, lambda)
    }
        ...
        for i, row in enumerate(reader, start=1):
            ...
            for group_name, cols in groups.items():
                for col in cols:
                    ...
                    # Track worst overall for the group
                    if diff > results[group_name]["overall_worst"][0]:
                        results[group_name]["overall_worst"] = (diff, i, col, filename, lam)

# --- Compute and print per-group results ---
all_diffs_combined = []  # to compute global stats later
global_worst = (0, None, None, None, None, None)  # (diff, group, row, col, file, lambda)
...
for group_name, data in results.items():
    ...
    # Track global worst diff
    if w[0] > global_worst[0]:
        global_worst = (w[0], group_name, w[1], w[2], w[3], w[4])
...
if all_diffs_combined:
    ...
    print(f"Global worst diff:      {global_worst[0]:.3f} "
          f"(group '{global_worst[1]}', row {global_worst[2]}, column '{global_worst[3]}', "
          f"file '{global_worst[4]}', lambda={global_worst[5]})")
else:
