# -*- coding: utf-8 -*-
# This file is copied from the Lattice Estimator project
...
class NoiseDistribution:
    ...
    def support_size(self, n=None, fraction=1.0):
        ...
        if not n:
            if not self.n:
                raise ValueError(
                    f"Length required to determine support size, but n was {n}.")
            n = self.n
    ...
    def get_hamming_weight(self, n=None):
        if not n:
            if not self.n:
                raise ValueError(
                    "Length required to determine hamming weight.")
            n = self.n
