...
all_params14 = [ [250.0, 64, 311, 7315, 17222, 8, 152.10], [300.0, 64, 243, 7105, 17690, 6, 135.44],
                 [400.0, 64, 197, 5496, 21313, 5, 113.43], [450.0, 64, 222, 3181, 26328, 6, 106.69],
                 [500.0, 64, 160, 4346, 23821, 4, 97.83],  [550.0, 64, 175, 2416, 27915, 5, 91.67],
    
                [700, 64, 116, 2253, 28240, 4, 76.15782878506808], [750, 64, 106, 1920, 28924, 4, 72.00766336007604],
                ...
                [250.0, 128, 417, 5299, 21819, 11, 190.64], [300.0, 128, 386, 3750, 25173, 11, 165.33],
                [350.0, 128, 363, 2236, 28353, 11, 146.87], [400.0, 128, 305, 1984, 28852, 9, 130.17],
                [450.0, 128, 262, 1714, 29399, 8, 117.08], [500.0, 128, 231, 1339, 30159, 7, 106.55],
                ...
                [250.0, 192, 532, 3231, 26322, 16, 208.80], [300.0, 192, 469, 1920, 29035, 15, 178.02],
                [350.0, 192, 388, 1578, 29718, 12, 153.90], [400.0, 192, 330, 1217, 30443, 11, 135.22],
                [450.0, 192, 280, 1073, 30722, 9, 120.75], [500.0, 192, 255, 391, 32118, 10, 108.88],
                ...
                [250.0, 256, 417, 5299, 21819, 11, 190.64], [300.0, 256, 386, 3750, 25173, 11, 165.33],
                [350.0, 256, 363, 2236, 28353, 11, 146.87], [400.0, 256, 305, 1984, 28852, 9, 130.17],
                [450.0, 256, 262, 1714, 29399, 8, 117.08], [500.0, 256, 231, 1339, 30159, 7, 106.55],
                ...
                [700, 512, 152, 58, 32780, 11, 78.95141727596588], [750, 512, 134, 63, 32745, 9, 73.64342664307411],
                [800, 512, 119, 59, 32758, 8, 69.25413802826124], [850, 512, 106, 59, 32776, 7, 65.44942196868422],
...
all_params15 = [ [700, 64, 186, 14350, 34934, 5, 126], [750, 64, 228, 10330, 43999, 6, 121],
                 ...
                 [700, 256, 403, 2224, 61191, 12, 158], [750, 256, 371, 1842, 61955, 11, 148], 
                 [800, 256, 335, 1984, 61657, 10, 139], [850, 256, 322, 924, 63807, 11, 131], 
                 [900, 256, 297, 812, 64024, 10, 123], [950, 256, 277, 563, 64537, 10, 116], 
                 [1000, 256, 258, 390, 64874, 10, 111], 
                 
                 [700, 512, 436, 589, 64588, 17, 163], [750, 512, 396, 473, 64771, 16, 151], 
                 [800, 512, 362, 392, 64946, 15, 141], [850, 512, 331, 361, 64973, 14, 132], 
                 [900, 512, 306, 241, 65241, 14, 125], [950, 512, 285, 0, 65712, -1, 119], 
...
all_params17 = [
                [2500.0, 64, 218, 58398, 137200, 4, 138.81], [2600.0, 64, 219, 55194, 144564, 4, 135.71],
                [2700.0, 64, 221, 51762, 152383, 4, 132.85], [2800.0, 64, 211, 51160, 153744, 4, 129.66],
                [2900.0, 64, 252, 38341, 182253, 5, 128.32], [3000.0, 64, 216, 44115, 169535, 4, 124.48],
                [3100.0, 64, 216, 41197, 175985, 4, 122.03], [3200.0, 64, 219, 37476, 184136, 4, 119.93],
                [3300.0, 64, 202, 39284, 180182, 4, 117.13], [3400.0, 64, 217, 32180, 195597, 4, 115.49],

                [800, 128, 699, 81027, 84065, 15, 350.09968802677224], [850, 128, 696, 77483, 92421, 15, 339.0132890998411],
                ...
                [2500, 128, 392, 23673, 213728, 8, 167.18], [2600, 128, 378, 22134, 216956, 8, 161.46],
                [2700, 128, 393, 14848, 232083, 9, 158.49], [2800, 128, 346, 20623, 220113, 7, 152.01],
                [2900, 128, 359, 13770, 234320, 8, 148.17], [3000, 128, 347, 12482, 236947, 8, 144.15],
                [3100, 128, 303, 19210, 223051, 6, 139.6], [3200, 128, 319, 11549, 238853, 7, 135.73],
...
def numerical_lambda_hybrid_runoptimize(n, logq, sigma_e, h, initial_guess):
    ...
    for wg in range(2,52):
        ...
        eq3 = lambda ng_, beta_, d_: (-d_+1)*log2(_delta(beta_))+ ((d_-n+ng_-1)*logq+ (n-ng_)*log2(xi))/d_ - log2(2*sigma_e*sigma_e)
        ...
        try:
            res = fsolve(system, [initial_guess[0], initial_guess[1], initial_guess[2]], maxfev = 2**21, full_output=False)
        except:
            print('error in numerical_lambda_hybrid_runoptimize on wg = ', wg)
            continue

        sol_tolerance = abs(eq1(res[0], res[1], res[2]))+abs(eq2(res[0], res[1], res[2]))+abs(eq3(res[0], res[1], res[2]))
...
def numerical_lambda_hybrid(n, logq, sigma_e, h, initial_guess = None):
    ...
    while bound_trials>0:
        bound_trials-=1
        rt = numerical_lambda_hybrid_runoptimize(n, logq, sigma_e, h, [n/4, beta_start, d_start-n/4])
        if not (rt == float('inf')): break
        beta_start = max(40, initial_guess[0]+random.randint(-3000,30))
        d_start = sqrt(2 * n * lnq * beta_start / log(beta_start / const))
        
        
    if bound_trials==0: 
        print("numerical_lambda_hybrid_runoptimize couldn't find a solution after", 200-bound_trials, "trial for this parameters set")
        return rt_min
...
def numerical_lambda_hybrid_v2(n, logq, sigma_e, h, initial_guess=None):
    ...
    sol_tolerance = 100
    for wg in range(2, 52):
        eq1 = lambda ng_, beta_, d_: (approx_binom(ng_, wg) + wg) + log2(d_) - (0.292 * beta_ + 16.4 + 3) + 2
        eq2 = lambda ng_, beta_, d_: d_ - ceil(sqrt(n * lnq / log(_delta(beta_)))) + ng_ - 1
        eq3 = lambda ng_, beta_, d_: (-d_ + 1) * log2(_delta(beta_)) + ((d_ - n + ng_ - 1) * logq + (n - ng_) * log2(xi)) / d_ - log2(2 * sigma_e * sigma_e)
        ...
        while bound_trials > 0:
            ...
            try:
                with warnings.catch_warnings(record=True) as w:
                    ...
                    res = fsolve(system, initial_guess, maxfev=2**21, full_output=False)
...
def numerical_logq_hybrid_runoptimize(n, l, sigma_e, h, initial_guess):
    ...
    for wg in range(2,55):
        eq1a = lambda ng_, beta_, d_: l - (0.292*beta_+16.4+3+log2(d_)) + approx_binom(n-h,ng_-wg)+approx_binom(h,wg) - approx_binom(n, ng_) - 1 #- 0.8028164000061224
        eq1b = lambda ng_, beta_, d_:  l - approx_binom(ng_, wg)- wg - 2*log2(d_) + approx_binom(n-h,ng_-wg)+approx_binom(h,wg) - approx_binom(n, ng_) - 12 #+ 1.8586100196043844
        eq2 = lambda ng_, beta_, d_, logq: d_ - ceil(sqrt(n * logq  /log_delta(beta_))) + ng_ - 1 #- 0.8388965696794912                                            #adding ceil over sqrt() makes the eq. precise -
        eq3 = lambda ng_, beta_, d_, logq: (-d_+1)*log_delta(beta_)+ ((d_-n+ng_-1)*logq+ (n-ng_)*log2(xi))/d_ - log2(2*sigma_e*sigma_e) #- 0.24517475390399568
        ...
        sol_tolerance = abs(eq1a(res[0], res[1], res[2]))+abs(eq1b(res[0], res[1], res[2]))+abs(eq2(res[0], res[1], res[2], res[3]))+abs(eq3(res[0], res[1], res[2], res[3]))
...
def numerical_logq_hybrid(n, l, h):
    ...
    bound_trials = 200 #if no candidates found, try again with new initial guess up to bound_trials  times
    while len(res[0])==0 and bound_trials>0:
        bound_trials-=1
        initial_guess = random.randint(30, 2500)
        res = numerical_logq_hybrid_runoptimize(n, l, 3.19, h, initial_guess)
        if len(res[0])>0: break

    if len(res[0])==0: 
        print("numerical_logq_hybrid_runoptimize couldn't find a solution after", 200-bound_trials, "trial for this parameters set")
        return float('inf')
...
def check_candidates_logq(logqs, h, l, n, best_diff, initial_guesses):
    ...
    if len(logqs)==1: return logqs[0]

    for i, logq in enumerate(logqs):
        l_ = numerical_lambda_hybrid_v2(n, logq, sigma_e, h, initial_guess=initial_guesses[i])
