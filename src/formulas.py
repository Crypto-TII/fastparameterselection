...
def log_gh(d, logvol=0):
    ...
    :param d: Dimension.
    :param logvol: Logarithm of the volume.
    :return: Gaussian Heuristic constant.
    """
    if d < 49:
...
def predicted_beta_bdd(n, q, sigma, zeta):
    ...
    :param zeta: Ratio of standard deviations.
    ...
    # print("beta approx", beta_approx1, "log(n/lnq)",
    #      math.log(n/(lnq)), "n/lnq", n/lnq, "n", n, "lnq", lnq)
...
def predicted_beta_bdd_rev1(n, q, sigma, zeta):
    lnq = math.log(q)

    beta_approx1 = const+1

    if n/(lnq) >= 1:
        beta_approx1 = max(const+1, 2*n/lnq*(math.log(n/(lnq))))  # very rough approximation
...
def predicted_beta_usvp(d, lnq, sig, chi):
    ...
    :param sig: Standard deviation of the error.
    :param chi: Ratio of standard deviations.
    :return: Predicted beta value.
    """
    x = np.divide(d, lnq)
...
def model_lambda_bdd(d, logq, std_s, std_e, params):
    ...
    :param std_s: Standard deviation of the secret.
    :param std_e: Standard deviation of the error.
    :param std_s_num: Numerical standard deviation of the secret.
    :param params: Model parameters.
    :return: Lambda value.
    """
    chi = std_e/std_s
    ...
    if isinstance(logq, list):
        for lq in logq:
            beta.append(predicted_beta_bdd_rev1(d, 2 ** lq, std_e, std_e/std_s))
            #beta.append(predicted_beta_bdd(d, 2 ** lq, std_e, std_e/std_s))
    else:
        beta.append(predicted_beta_bdd_rev1(d, 2 ** logq, std_e, std_e/std_s))
