...
def run_verification(lq, secret, est_usvp, est_bdd, est_usvp_pow, est_bdd_pow):
    ...
    lwe_usvp = math.floor(math.log2(LWE.primal_usvp(lwe_parameters_usvp, red_cost_model=RC.BDGL16)["rop"]))
    lwe_bdd = math.floor(math.log2(LWE.primal_bdd(lwe_parameters_bdd, red_cost_model=RC.BDGL16)["rop"]))
    lwe_usvp_pow = math.floor(math.log2(LWE.primal_usvp(lwe_parameters_usvp_pow, red_cost_model=RC.BDGL16)["rop"]))
    lwe_bdd_pow = math.floor(math.log2(LWE.primal_bdd(lwe_parameters_bdd_pow, red_cost_model=RC.BDGL16)["rop"]))
...
def process_parameters(params):
    ...
    if param == 'n':
        headers, data = process_n(logq, l, std_s, std_e, model_values['n_usvp'], model_values['n_usvp_s'], model_values['n_bdd'], model_values['n_bdd_s'], verify, estimator_installed, secret, secret_q, output_dict)
    elif param == 'logq':
        headers, data = process_logq(l, lwe_d, std_s, std_e, verify, estimator_installed, secret, secret_q, hw, output_dict)
    elif param == 'std_e':
        headers, data = process_std_e(logq, l, lwe_d, std_s, verify, estimator_installed, secret, secret_q, output_dict)
    elif param == 'lambda':
        headers, data = process_lambda(logq, lwe_d, std_s, std_e, model_values['lambda_usvp'], model_values['lambda_usvp_s'], model_values['lambda_bdd'], model_values['lambda_bdd_s'], verify, estimator_installed, secret, secret_q, hw, output_dict)
    elif param == "est":
    ...
    for i, row in enumerate(data):
        if param == 'std_e': 
            ...
            if row[4] <= 1 or row[5] <= 1: 
                ...
                print("python3 src/estimate.py --param \"{}\" --n \"{}\" --logq \"{}\" --secret \"{}\" --error \"{}\"".format("est", row[2], row[3], secret, 2**row[4]))
                print("python3 src/estimate.py --param \"{}\" --n \"{}\" --logq \"{}\" --secret \"{}\" --error \"{}\" \n".format("est", row[2], row[3], secret, 2**row[5]))
        elif param == 'n': 
...
def process_n_param(logq, l, std_s, std_e, n_usvp, n_usvp_s, n_bdd, n_bdd_s, verify, estimator_installed, secret, secret_q, output_dict):
    ...
    for lq in logq:
        ...
        est_usvp_numerical = int(math.ceil(numerical_n_usvp(l, lq, std_s, std_e)))
        est_bdd_numerical = int(math.ceil(numerical_n_bdd(l, lq, std_s, std_e)))
        
        optimal_value = max(est_usvp, est_usvp_s, est_bdd, est_bdd_s, est_usvp_numerical, est_bdd_numerical) # Store the minimum value of n provided from all the formulas
        if len(logq) > 1:
        ...
        if verify and estimator_installed:
            lwe_parameters_usvp = LWE.Parameters(est_usvp, 2 ** lq, ND.UniformMod(secret_q), ND.DiscreteGaussian(std_e))
            lwe_parameters_bdd = LWE.Parameters(est_bdd, 2 ** lq, ND.UniformMod(secret_q), ND.DiscreteGaussian(std_e))
            lwe_parameters_usvp_s = LWE.Parameters(est_usvp_s, 2 ** lq, ND.UniformMod(secret_q), ND.DiscreteGaussian(std_e))
            lwe_parameters_bdd_s = LWE.Parameters(est_bdd_s, 2 ** lq, ND.UniformMod(secret_q), ND.DiscreteGaussian(std_e))
            lwe_parameters_usvp_num = LWE.Parameters(est_usvp_numerical, 2 ** lq, ND.UniformMod(secret_q), ND.DiscreteGaussian(std_e))
            lwe_parameters_bdd_num = LWE.Parameters(est_bdd_numerical, 2 ** lq, ND.UniformMod(secret_q), ND.DiscreteGaussian(std_e))

            lwe_usvp = math.floor(math.log2(LWE.primal_usvp(lwe_parameters_usvp, red_cost_model=RC.BDGL16)["rop"]))
            lwe_bdd = math.floor(math.log2(LWE.primal_bdd(lwe_parameters_bdd, red_cost_model=RC.BDGL16)["rop"]))
            lwe_usvp_s = math.floor(math.log2(LWE.primal_usvp(lwe_parameters_usvp_s, red_cost_model=RC.BDGL16)["rop"]))
            lwe_bdd_s = math.floor(math.log2(LWE.primal_bdd(lwe_parameters_bdd_s, red_cost_model=RC.BDGL16)["rop"]))
            lwe_usvp_numerical = math.floor(math.log2(LWE.primal_usvp(lwe_parameters_usvp_num, red_cost_model=RC.BDGL16)["rop"]))
            lwe_bdd_numerical = math.floor(math.log2(LWE.primal_bdd(lwe_parameters_bdd_num, red_cost_model=RC.BDGL16)["rop"]))

            data_point = [secret, l, lq, est_usvp, lwe_usvp, est_usvp_s, lwe_usvp_s, est_usvp_numerical, lwe_usvp_numerical, est_bdd, lwe_bdd, est_bdd_s, lwe_bdd_s, est_bdd_numerical, lwe_bdd_numerical, optimal_value, closest_power_of_2(optimal_value)]
        else:
            data_point = [secret, l, lq, est_usvp, est_usvp_s, est_usvp_numerical, est_bdd, est_bdd_s, est_bdd_numerical, optimal_value, closest_power_of_2(optimal_value)]
...
def process_logq_param(l, lwe_d, std_s, std_e, verify, estimator_installed, secret, secret_q, output_dict):
    ...
    if verify and estimator_installed:
        ...
        lwe_bdd = math.floor(math.log2(LWE.primal_bdd(lwe_parameters_bdd, red_cost_model=RC.BDGL16)["rop"]))
        lwe_usvp = math.floor(math.log2(LWE.primal_usvp(lwe_parameters_usvp, red_cost_model=RC.BDGL16)["rop"]))
        data_point = [secret, l, lwe_d, est_usvp_numerical, est_bdd_numerical, lwe_usvp, lwe_bdd, optimal_value]
    else:
        data_point = [secret, l, lwe_d, est_usvp_numerical, est_bdd_numerical, optimal_value]
    data.append(data_point)
...
def process_logq_param_hybrid(l, lwe_d, std_s, std_e, verify, estimator_installed, secret, secret_q, h, output_dict):
    ...
    if verify and estimator_installed:
        ...
        primal_hybrid_cost = math.floor(math.log2(LWE.primal_hybrid(FHEParam, red_cost_model=RC.BDGL16, mitm=False)["rop"]))
...
def process_std_e_param(logq, l, lwe_d, std_s, verify, estimator_installed, secret, secret_q, output_dict):
    ...
    for lq in logq:
        ...
        if verify and estimator_installed:
            lwe_parameters_bdd = LWE.Parameters(lwe_d, 2 ** lq, ND.UniformMod(secret_q), ND.DiscreteGaussian(2**est_bdd_numerical))
            lwe_parameters_usvp = LWE.Parameters(lwe_d, 2 ** lq, ND.UniformMod(secret_q), ND.DiscreteGaussian(2**est_usvp_numerical))
            lwe_bdd = math.floor(math.log2(LWE.primal_bdd(lwe_parameters_bdd, red_cost_model=RC.BDGL16)["rop"]))
            lwe_usvp = math.floor(math.log2(LWE.primal_bdd(lwe_parameters_usvp, red_cost_model=RC.BDGL16)["rop"]))
            data_point = [secret, l, lwe_d, lq, est_usvp_numerical, est_bdd_numerical, lwe_usvp, lwe_bdd, optimal_value]
        else:
            data_point = [secret, l, lwe_d, lq, est_usvp_numerical, est_bdd_numerical, optimal_value]
        data.append(data_point)
...
def process_lambda_param(logq, lwe_d, std_s, std_e, lambda_usvp, lambda_usvp_s, lambda_bdd, lambda_bdd_s, verify, estimator_installed, secret, secret_q, output_dict):
    ...
    """
    data = []
    if len(logq) > 1: output_dict['lambda'] = []
    for lq in logq:
        est_usvp, est_bdd = estimate_usvp_bdd(lwe_d, lq, std_s, std_e, lambda_usvp, lambda_bdd, secret_q)
        if abs(std_e - 3.19) < 1e-9:
            est_usvp_s, est_bdd_s = estimate_usvp_s_bdd_s(lwe_d, lq, lambda_usvp_s, lambda_bdd_s)
            optimal_value = max(est_usvp, est_usvp_s, est_bdd, est_bdd_s)
            data_point = create_data_point_with_verification(verify, estimator_installed, lq, secret, lwe_d, est_usvp, est_usvp_s, est_bdd, est_bdd_s, optimal_value)
        else:
            optimal_value = max(est_usvp, est_bdd)
            data_point = create_data_point_without_verification(verify, estimator_installed, lq, secret, lwe_d, est_usvp, est_bdd, optimal_value)
